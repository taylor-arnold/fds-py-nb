...
2. As in the notes, create a variable called `my_fav_number` equal to your favorite number. Note: See the font that the variable is in? Those come from putting the variable in back quotes within the text. Those are used to indicate that something in the prose text is a Python variable, function, or module. You will see that in the solutions these get printed in a code-font as well.
...
3. Now, use the function `np.log`, a function that comes from the numpy module to compute logarithms, to find the logarithm of 10.
