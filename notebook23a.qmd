...
```{python}
#| tags: [noclear]
plays = pl.read_csv(ub + "data/shakespeare_lines.csv.gz")
...
By the end you will have a model that can accept a short prompt and generate (somewhat coherent) news-like text, all trained on the Shakespeare dataset.
...
print("Input tokens (x): ", x_sample[:10].tolist(), "...")
print("Target tokens (y):", y_sample[:10].tolist(), "...")
print()
print("In other words, given this text:")
print(f"  '{enc.decode(x_sample[:10].tolist())}'")
print("the model should predict this next token:")
print(f"  '{enc.decode([y_sample[9].item()])}'")
```
...
Now the actual training, here is the function that we will use to train the model using our data.
...
Let's try a few prompts. Remember, this model has only seen Shakespeare's text, so it will tend to produce text in a similar way and format.
...
for prompt in prompts:
    ...
    print(generate(model, prompt, max_new_tokens=100))
    print()
    print("---")
    print()
```
